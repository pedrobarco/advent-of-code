...
fn blink(stone: Stone) -> List(Stone) {
  ...
  case stone {
    0 -> [1]
    _ if is_even -> split_string(stone_str) |> list.map(utils.parse_unsafe_int)
    x -> [x * 2024]
  }
}
    ...
    _ -> {
      case dict.get(cache, #(stone, steps)) {
        ...
        _ -> {
          ...
          let #(next_cache, next_nstones) =
            blink_dp(next_stones, steps - 1, cache)
          let next_new_cache =
            dict.insert(next_cache, #(stone, steps), next_nstones)
